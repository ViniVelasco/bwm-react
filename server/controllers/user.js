...
exports.auth = function(req, res) {
  ...
  User.findOne({email}, function(err, user){
    ...
    if(user.isSamePassword(password)) {
      ...
      return res.status(422).send({errors: [{title: 'Wrong Data!', detail: 'Wrong e-mail or password'}]})
    }
...
exports.register = function(req, res) {
  const {username, email, password, passwordConfirmation} = req.body; //destructuring
